// This code was copied from https://github.com/aszczepanski/2048 and ported for my task
...
// Only ported code above and no additions for my task.
// Below is my code.
...
inline static board_t draw_tile_urng(URNG & urng) {
    return (urng.next<int>(0, 9) < 9) ? 1 : 2;
}

inline static board_t insert_tile_urng(board_t board, board_t tile, URNG & urng) {
    int index = urng.next<int>(0, count_empty(board) - 1);
    board_t tmp = board;
    while (true) {
        while ((tmp & 0xf) != 0) {
            tmp >>= 4;
...
inline int nnPlay(board_t board, NeuralNetwork * nn, std::vector<double> & xVec) {
	...
	double * x = xVec.data();
	const int vBytesCount = xVec.size()*sizeof(double);

    for (int move = 0; move < 4; ++move) {
        if ((temp = execute_move(move, board)) != board) {
			memset(x, 0, vBytesCount); // faster than std::fill
			//std::fill(x.begin(), x.end(), 0);
			for (register int i = 0; i < 16; ++i) {
				x[(temp & 0xf)*16 + i] = 1;
				temp >>= 4;
			}
			//x[16*16] = count_empty(temp)/16.0;
			moveWeight = *(nn->propogate(xVec));
			if (maxWeight < moveWeight) {
...
int fastPlayGame(NeuralNetwork * nn, URNG & gen) {

	board_t board = initial_board();
    int scorepenalty = 0; // "penalty" for obtaining free 4 tiles

	//std::vector<double> x(257); // zero tiles count was 257'th feature
	std::vector<double> x(256);

	int move;
	board_t tile;
    while (1) {
        if ((move = nnPlay(board, nn, x)) < 0) break;
        if ((tile = draw_tile_urng(gen)) == 2) scorepenalty += 4;
		board = insert_tile_urng(execute_move(move, board), tile, gen);
    }

	// print "#" if we had 2048 tile
	for (int i = 0; i < 16; ++i) {
		if (((board >> i*4) & 0xf) >= 11) {
			printf("#");
		}
	...
	int score = (int)score_board(board) - scorepenalty;
	if (score > maxScore) {
		maxScore = score;
		printf("\nScore: %d\n", score);
		print_board(board);
...
std::vector<double> threadScores(threadsCount);

inline void playRecursive(NeuralNetwork * nn, board_t board, const int index,
						   URNG & random, std::vector<double> & x) {
	board = insert_tile_urng(board, draw_tile_urng(random), random);
	int move;
    while (1) {
        if ((move = nnPlay(board, nn, x)) < 0) break;
	...
	threadScores[index] += score_board(board);
}

void playRandom(NeuralNetwork * nn, board_t board, const int gamesCount, const int index) {

	URNG random;
	std::vector<double> x(257);

	threadScores[index] = 0;
	for (int i = 0; i < gamesCount; ++i) {
		playRecursive(nn, board, index, random, x);
	}
	threadScores[index] /= gamesCount;
}

int fastMonteCarlo(std::vector<NeuralNetwork *> & nns, URNG & random) {
	...
	board_t tile, newBoard;

	double moveWeight, maxWeight;
	int move;
    ...
    while (1) {
		...
		for (int m = 0; m < 4; ++m) {
			if ((newBoard = execute_move(m, board)) != board) {
				moveWeight = 0;
				for (size_t t = 0; t < threadsCount; ++t) {
					threads[t] = std::thread(playRandom, nns[t], newBoard, 50, t);
				}
				for (size_t t = 0; t < threadsCount; ++t) {
					threads[t].join();
				}
				for (size_t t = 0; t < threadsCount; ++t) {
					moveWeight += threadScores[t];
				}
				moveWeight /= 4;
				if (maxWeight < moveWeight) {
					maxWeight = moveWeight;
		...
		board = execute_move(move, board);
        if ((tile = draw_tile_urng(random)) == 2) scorePenalty += 4;
		board = insert_tile_urng(board, tile, random);
		print_board(board);
    }
...
int fastHardcode(std::vector<NeuralNetwork *> & nns, URNG & random) {
	...
	while (1) {
		...
		for (int m = 0; m < 4; ++m) {
			if ((newBoard = execute_move(m, board)) != board) {
				moveWeight = 0;
				for (size_t t = 0; t < threadsCount; ++t) {
					threads[t] = std::thread(playRandom, nns[t], newBoard, 8, t);
				}
				for (size_t t = 0; t < threadsCount; ++t) {
					threads[t].join();
				}
